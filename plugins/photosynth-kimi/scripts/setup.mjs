#!/usr/bin/env node
// Photosynth for Kimi Code CLI — setup script.
// Registers the wallet (Arkreen membership check) → obtains an auth_token →
// writes ~/.kimi/photosynth/config.json. The Stop hook then reads that config.
...
async function main() {
  ...
  const config = {
    ...
    schema_version: '1',
    plugin_version: '0.1.0-kimi',
  };
  fs.mkdirSync(DIR, { recursive: true });
  writeJsonAtomic(CONFIG_PATH, config);

  console.log('✅ Photosynth for Kimi configured.');
  ...
  console.log('');
  console.log('Make sure your ~/.kimi/config.toml has the Stop hook installed:');
  console.log('  [[hooks]]');
  console.log('  event = "Stop"');
  console.log(`  command = "node ${path.join(DIR, 'hook.mjs')}"`);
